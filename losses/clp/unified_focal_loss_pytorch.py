...
class AsymmetricFocalLoss(nn.Module):
	...
	# Calculate losses separately for each class, only suppressing background class
        if cross_entropy.shape[1] == 2: # background and foreground in 2 different layers
            back_ce = torch.pow(1 - y_pred[:,0,:,:], self.gamma) * cross_entropy[:,0,:,:]
            back_ce =  (1 - self.delta) * back_ce
            ...
            cross_entropy_bk = -(1-y_true) * torch.log(1-y_pred)
            back_ce = torch.pow(y_pred[:,0,:,:], self.gamma) * cross_entropy_bk[:,0,:,:]
            back_ce =  (1 - self.delta) * back_ce
...
class AsymmetricFocalTverskyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        if dice_class.shape[1] == 2: # background and foreground in 2 different layers
            back_dice = (1-dice_class[:,0]) 
            fore_dice = (1-dice_class[:,1]) * torch.pow(1-dice_class[:,1], -self.gamma) 
        else:
            dice_class_bk = (tn + self.epsilon)/(tn + self.delta*fp + (1-self.delta)*fn + self.epsilon)
            back_dice = (1-dice_class_bk[:,0]) 
            fore_dice = (1-dice_class[:,-1]) * torch.pow(1-dice_class[:,-1], -self.gamma) 
...
class AsymmetricUnifiedFocalLoss(nn.Module):
    ...
    def __init__(self, weight=0.5, delta=0.6, gamma=0.2):
        ...
        self.aftloss = AsymmetricFocalTverskyLoss(delta=self.delta, gamma=self.gamma)
        self.afloss = AsymmetricFocalLoss(delta=self.delta, gamma=self.gamma)

    def forward(self, y_pred, y_true):
      # Obtain Asymmetric Focal Tversky loss
      asymmetric_ftl = self.aftloss(y_pred, y_true)

      # Obtain Asymmetric Focal loss
      asymmetric_fl = self.afloss(y_pred, y_true)
